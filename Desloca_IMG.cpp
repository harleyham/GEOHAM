...
void MainWindow::Ok_Desloca() {
    ...
    if (dialogDeslocaImg->opcao() == 1) {
        dialogDeslocaImg->getOldLatLon(OldLat,OldLon);
    } else if (Geo_Referencia.Pontos.length()) {
...
//    Delta_Lon = PNew.Longitude() - POld.Longitude();
    ...
    Delta_Lat = NewLat - OldLat;
    Delta_Lon = NewLon - OldLon;
...
//   MAT2GDAL(New_File,Destino,NULL,Geo_Img);
    QFile::copy(Informacao.Imagem_Foco,New_File);
    GDALDataset *pOldDS;
    pOldDS = (GDALDataset *) GDALOpen(New_File.toStdString().c_str(),GA_Update);
    double transform[6] = {0,1,0,0,0,1};

    /////////////////////////////////////////////////////////////////////////////////
    Geometria_Imagem    Old,New;
    double              Est,Nor;

    pOldDS->GetGeoTransform(transform);

    Old.AUTM.Reset(transform[3],transform[0]); // LAT,LON
    Old.CcUTM.Reset(OldLat,OldLon);
    New.CcUTM.Reset(NewLat,NewLon);
    Est = New.CcUTM.Easting() - Old.CcUTM.Easting();
    Nor = New.CcUTM.Northing() - Old.CcUTM.Northing();
    New.AUTM.Reset(Old.AUTM.Zone(),Old.AUTM.Northp(),Old.AUTM.Easting() + Est,Old.AUTM.Northing() + Nor);
    transform[3] = New.AUTM.Latitude();
    transform[0] = New.AUTM.Longitude();
...
*/
    ...
    QString Output;
    Size_GeoTiff(New_File,true,Output);
...
void MainWindow::Desloca_Digita() {
    ...
    QString Output;
    Size_GeoTiff(Tif_Original,true,Output);
...
void MainWindow::Le_Imagem_TIF(QString InputTif,Geometria_Imagem &Geo_Tiff) {
    ...
    if (!E_UTM) {  // LAT LON
        ...
        Geo_Tiff.x_Size = Transform[1];
        Geo_Tiff.y_Size = Transform[5];
        Geo_Tiff.CcUTM.Reset(Geo_Tiff.AUTM.Latitude() + Geo_Tiff.y_Size * (Geo_Tiff.Iw /2),Geo_Tiff.AUTM.Longitude() + Geo_Tiff.x_Size * (Geo_Tiff.Ih / 2));
