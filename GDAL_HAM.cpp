...
void MainWindow::Size_GeoTiff(QString InputFile, bool Print, QString &Output) {
    ...
    if (!E_UTM) {  // LAT LON
        ...
        Geo_Img.x_Size = Transform[1];
        Geo_Img.y_Size = Transform[5];
        Geo_Img.CcUTM.Reset(Geo_Img.AUTM.Latitude() + Geo_Img.y_Size * (Geo_Img.Iw /2),Geo_Img.AUTM.Longitude() + Geo_Img.x_Size * (Geo_Img.Ih / 2));
