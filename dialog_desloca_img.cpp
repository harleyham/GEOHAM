...
void Dialog_Desloca_Img::printOriginalCoords(Geometria_Imagem &GeoProc) {
    ...
    if (GeoProc.Pontos.length()) {
        QLocale Br;
        char    intStr[30];
        OldLat = GeoProc.Pontos.at(0).real();
        OldLon = GeoProc.Pontos.at(0).imag();
...
void Dialog_Desloca_Img::getOldLatLon(double &Lat, double &Lon) {
    Lat = OldLat;
    Lon = OldLon;
}
...
int Dialog_Desloca_Img::opcao(void) {
    if (ui->Bt_Digita->isChecked()) {
        return 1;
    } else if (ui->Bt_Img->isChecked()) {
        return 2;
    } else if (ui->bBt_2Pontos->isChecked()) {
        return 3;
    } else return 0;
}
